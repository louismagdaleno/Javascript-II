...
// Problem 1
// Find the total number of donations made by people wearing large shirts
...
// Problem 2
// Display a list of email addresses of participants who wore size M shirts and donated 50 or greater.
let highRollers = [];
highRollers = 
    runners.filter(runner => runner.shirt_size === 'M' && runner.donation >= 50)
    .map(runner => runner.email);
    
console.log(highRollers);

// Problem 3
// Find the total donations by shirt sizes and display the results

let totalShirtDonations = [];

// create unique list of shirt sizes and an iterator
...
for (let i = 0; i < sizes.size; i++) {
    console.log(`${iterator.next().value} : ${totalShirtDonations[i]}`);
}